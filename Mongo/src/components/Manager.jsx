...
const Manager = () => {
    ...
    const savePassword = async () => {
        if (form.site.length > 3 && form.username.length > 3 && form.password.length > 3) {
            const id = form.id || uuidv4(); // Use existing ID if editing, otherwise generate new one
            const newForm = { ...form, id };
    
            // Update local state first
            setPasswordArray([...passwordArray.filter(item => item.id !== id), newForm]);
    
            // Send DELETE request if editing an existing password
            if (form.id) {
            ...
            // Send POST request to save the new or updated password
            await fetch("http://localhost:3000/", { 
                method: "POST", 
                headers: { "Content-Type": "application/json" }, 
